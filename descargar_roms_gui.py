...
import getpass # Para obtener usuario actual (opcional)
...
def run_scp_transfer(ip, user, password, local_base_path, local_console_dirs):
    ...
    try:
        ...
        for console_dir in local_console_dirs:
            ...
            # Listar archivos remotos (optimización: obtener una vez si es posible)
            ...
            try:
                ...
                remote_files_set = {item.filename for item in remote_listing if stat.S_ISREG(item.st_mode)}
            except (paramiko.SFTPError, IOError) as e:
            ...
            for filename in local_files:
                ...
                if filename in remote_files_set:
                    # print(f"Saltando (ya existe): {remote_file_path}")
                    skipped_count += 1
                else:
                    try:
                        update_status(f"[{progress_perc}%] Copiando {console_dir}/{filename[:30]}...")
                        sftp.put(local_file_path, remote_file_path)
                        # print(f"Copiado: {local_file_path} -> {remote_file_path}")
                        copied_count += 1
                        ...
                        error_count += 1
                        # Intentar eliminar archivo parcial remoto si falló la copia? (Complejo)
        ...
        def reactivate_buttons():
            ...
            if error_count == 0 and copied_count == 0 and skipped_count == 0 and not errors_list:
                 ...
                 status_label.config(text=f"Copia a RetroPie completada. Copiados: {copied_count}, Omitidos: {skipped_count}.")
